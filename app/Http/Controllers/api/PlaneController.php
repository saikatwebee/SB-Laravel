...
'.$slack_assign_name,
    ...
    public function ApplyCoupon( Request $request ) {
        try {
            ...
            if ( !empty( $code ) ) {
                $couponcheck = InvoiceService::checkCoupon( $code, $cid );
                if ( $code == $coupon ) {
					...
					$data[ 'customerInfo' ] = CommonService::getRowByCid( $cid );
					$data[ 'firstname' ] = $data[ 'customerInfo' ]->firstname;
					$data[ 'lastname' ] = ( $data[ 'customerInfo' ]->lastname ) ? $data[ 'customerInfo' ]->lastname : '';
					$data[ 'address' ] = ( $data[ 'customerInfo' ]->address ) ? $data[ 'customerInfo' ]->address : '';
					$data[ 'city' ] = ( $data[ 'customerInfo' ]->city ) ? $data[ 'customerInfo' ]->city : '';
                    ...
                    $plane = $data[ 'planeInfo' ]->id;

                    if ($plane == 36 || $plane == 25 ) {
						$type = 'silver';
                    } else if ( $plane == 37 || $plane == 26) {
						$type = 'gold';
                    } else if ( $plane == 38 || $plane == 27) {
						$type = 'platinum';
                    }
					else{
						$type = 'silver';
                    ...
                    $discount = InvoiceService::getCouponDiscount( $code, $cid, $type );
                    ...
                    return response()->json( [ 'message' => 'Please Check your coupon code again!' ], 502 );
                }

            } else {
                return response()->json( [ 'message' => 'Coupon code is required!' ], 502 );
            }
